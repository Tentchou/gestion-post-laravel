<x-default-layout title="{{ $post->title }}">
        ...
        <div class="space-y-10 md:space-y-16">
            ...
            <x-post :$post/>
        {{-- Fin du post --}}
         {{-- ajouter un commentaire sachant que l'on doits'authentifier pour le faire --}}
           @auth
        ...
        {{-- affichage de notre commentaire avec une boucle --}}
                ...
                <div class="flex bg-slate-50 p-6 rounded-lg">
                    ...
                    <div class="ml-4 flex flex-col">
                        <div class="flex flex-col sm:flex-row sm:items-center">
                            <h2 class="font-bold text-slate-900 text-xl">{{ $comment->user->name }}</h2>
                            <time class="mt-2 sm:mt-0 sm:ml-4 text-xs text-slate-400" datetime="{{ $comment->created_at }}">@datetime($comment->created_at)</time>
                        </div>
                        <p class="mt-5 text-slate-800 sm:leading-loose">{{ $comment->content }}</p>
                    </div>
                </div>
            @endforeach
        </div>
