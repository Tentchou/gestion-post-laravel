...
<article class="flex flex-col lg:flex-row pb-10 md:pb-16 border-b">
    ...
    <div class="flex flex-col items-start mt-5 space-y-5 lg:w-7/12 lg:mt-0 lg:ml-12">
        @if ($post->category)
           <a href="{{ route('posts.byCategorie', $post->category->id) }}" class="underline font-bold text-slate-900 text-lg">{{ $post->category->name }}</a>
        @endif
  ...
  {{-- $tag-> --}}
                   <li><a href="{{ route('posts.byTag', $tags->id) }}" class="px-3 py-1 bg-indigo-700 text-indigo-50 rounded-full text-sm">{{ $tags->name }}</a></li>
                @endforeach
            </ul>
        ...
        @if ($list)
            <a href="{{ route('posts.show', $post->id) }}" class="flex items-center py-5 px-7 font-semibold bg-slate-900 transition text-slate-50 rounded-full">
